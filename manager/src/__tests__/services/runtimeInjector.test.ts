...
import { CursorAgentProvider, AgentProvider } from "../../providers";
...
const providers: AgentProvider[] = [defaultProvider];
...
describe("RuntimeInjector", () => {
  ...
  describe("ensureSharedServicesMountOverride", () => {
    it("writes override YAML for workspace-level pool", async () => {
      ...
      const injector = new RuntimeInjector(config, providers);
  ...
  describe("getAppServiceContainerId", () => {
    it("returns trimmed container ID", async () => {
      ...
      const injector = new RuntimeInjector(makeConfig(), providers);
  ...
  describe("injectIntoAppContainer", () => {
    ...
    it("applies config and install scripts for each configured provider", async () => {
      const extraProvider: AgentProvider = {
        name: "mock",
        buildInstallScript: () => "echo install-mock-provider",
        buildConfigScript: () => "echo config-mock-provider",
        createThread: async () => "thread",
        runChat: async () => ({ threadId: "thread", stdout: "", stderr: "" }),
        getServicePaths: () => ({
          configLinkSource: "mock/default",
          configLinkTarget: "/root/.config/mock",
          conversationSubdir: "mock",
          dotDir: "dot-mock",
          dotDirTarget: "/root/.mock"
        }),
        getAvailableModels: () => []
      };
      mockRunCommand.mockResolvedValue({ stdout: "ok\n", stderr: "" });
      const injector = new RuntimeInjector(makeConfig(), [defaultProvider, extraProvider]);

      await injector.injectIntoAppContainer("abc123", "my-app", "ws");

      const args = mockRunCommand.mock.calls[0]?.[1] as string[];
      const script = args[4] || "";
      expect(script).toContain("GIT_TARGET_VERSION='2.53.0'");
      expect(script).toContain("cannot install git 2.53.0 as non-root");
      expect(script).toContain("config-mock-provider");
      expect(script).toContain("install-mock-provider");
    });
  });
});
