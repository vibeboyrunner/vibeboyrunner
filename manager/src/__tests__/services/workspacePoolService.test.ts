...
describe("WorkspacePoolService", () => {
  ...
  describe("runAgent", () => {
    ...
    it("streams thinking as a contiguous block", async () => {
      mockRunCommandStreaming.mockImplementation(async (_command, _args, options) => {
        options?.onStdout?.('{"type":"thinking","subtype":"delta","text":"Thinking "}\n');
        options?.onStdout?.('{"type":"thinking","subtype":"delta","text":"block text"}\n');
        options?.onStdout?.(
          '{"type":"tool_call","subtype":"started","tool_call":{"shellToolCall":{"args":{"command":"pwd"}}}}\n'
        );
        options?.onStdout?.(
      ...
      expect(stderrText).toContain("[tool:start] shell");
      expect(stderrText).not.toContain("[thinking] Thinking");
    });
  });
});
