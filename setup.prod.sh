...
PROD_ENV_EXAMPLE="${SCRIPT_DIR}/.env.prod.example"
COMMAND="up"
if [ "$#" -gt 0 ]; then
  case "$1" in
    -*)
...
if [ "$COMMAND" = "help" ] || [ "$COMMAND" = "--help" ] || [ "$COMMAND" = "-h" ]; then
  cat <<'EOF'
Usage: ./setup.prod.sh [command] [--image image-ref]
...
if [ ! -f "$ENV_FILE" ]; then
  if [ ! -f "$PROD_ENV_EXAMPLE" ]; then
    echo "Missing prod env template: $PROD_ENV_EXAMPLE" >&2
    exit 1
  fi
  cp "$PROD_ENV_EXAMPLE" "$ENV_FILE"
  echo "Created .env from $(basename "$PROD_ENV_EXAMPLE")"
fi
...
DIND_IMAGE_REF="${IMAGE_ARG:-${DIND_IMAGE_REF:-vbr-dind:local}}"
