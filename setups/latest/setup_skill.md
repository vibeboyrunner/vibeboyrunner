...
When sharing URLs, use the format that works best for the scheme.

- For `https://` and `http://`: use clickable markdown links `[label](url)`.
- For `cursor://` deeplinks: prefer raw plain-text URI first (copy/paste friendly), and optionally include a markdown link as secondary.
- For critical links, always include raw URL in inline code as fallback copy/paste.

## Response Formatting Rule

All user-facing responses must use a unified, easy-to-scan markdown style.

Required layout (when relevant for the current step):

1. `## Status` - one short sentence about current progress.
2. `## What to do now` - concise bullets with the immediate action.
3. `## Commands` - include only when the user must run terminal commands.
4. `## What to expect` - one short line describing success criteria.

Formatting requirements:

- Use markdown headers (`##` / `###`) to separate sections.
- Use *italics* for short emphasis only.
- Use quote blocks (`>`) for important warnings or notes.
- Put terminal commands in fenced `bash` blocks.
- Keep command blocks copy-paste ready and in execution order.
- Prefer short bullets over long paragraphs.

Command outlining rule:

- For each command block, add one short line before it explaining purpose.
- Add one short line after it: `Expected result: ...`
...
**Do NOT attempt to check or install this extension programmatically** — the `cursor` CLI extension commands are unreliable across platforms. Instead, ask the user to verify it themselves.

Tell the user:

> Please make sure the **Dev Containers** extension is installed in Cursor:
> 1. Open **Cursor IDE**.
> 2. Open Extensions: **Cmd+Shift+X** (macOS) or **Ctrl+Shift+X** (Windows/Linux).
> 3. Search for **Dev Containers** (by Anysphere).
...
```
curl -fsSL "https://vibeboyrunner.github.io/vibeboyrunner/setups/v0.0.1/setup.sh" | bash -s install
```
...
If you are in terminal mode and can run the command yourself:

- Parse the `cursor://...` deeplink from command output.
- Return the deeplink as raw text first (this is the primary format):
  - `cursor://...`
- Optionally also include a clickable markdown variant:
  - `[Open in Cursor](cursor://...)`
- In the same message, include this immediate next-step hint so the user does not need to return to setup chat:
  - "After Cursor opens `/workdir`, open Agent chat (`Cmd+L` / `Ctrl+L`) and say: **Hey, let's start onboarding!**"
...
When sharing this with the user, always present it as:

- `cursor://vscode-remote/attached-container+<hex>/workdir?windowId=_blank`
- Optional secondary format: `[Open in Cursor](cursor://vscode-remote/attached-container+<hex>/workdir?windowId=_blank)`

If you cannot determine the exact deeplink value, ask the user to copy the full `cursor://...` line from `vibeboyrunner up` output and re-share it as raw text first.

When sharing the deeplink, also include this hint in the same message:

- After opening Cursor and attaching to `/workdir`, open Agent chat (`Cmd+L` / `Ctrl+L`) and say: **Hey, let's start onboarding!**
...
2. Say exactly: **Hey, let's start onboarding!** — this explicitly asks the Father Agent to run onboarding.
