...
```
curl -fsSL "https://vibeboyrunner.github.io/vibeboyrunner/setups/v0.0.20/setup.sh" | bash -s install
```
...
- Return it to the user as a clickable markdown link in this exact format:
  - `[Open in Cursor](cursor://...)`
- Also include the raw deeplink in inline code as fallback copy/paste.
...
- `[Open in Cursor](cursor://vscode-remote/attached-container+<hex>/workdir?windowId=_blank)`

If you cannot determine the exact deeplink value, ask the user to copy the full `cursor://...` line from `vibeboyrunner up` output and then re-share it as a markdown link.
